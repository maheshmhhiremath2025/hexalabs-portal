...
const AwsSandbox = ({ superadminApiRoutes }) => {
    ...
    const fetchUsers = useCallback(async (silent = false) => {
        ...
        try {
            const res = await apiCaller.get(superadminApiRoutes.awsUserApi);
            setUsers(res.data);
        } catch {
        ...
        <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden" style={{ boxShadow: '0 1px 2px rgba(0,0,0,0.04)' }}>
                ...
                {loading ? (
                    ...
                    <div className="px-5 py-10 text-center text-sm text-gray-400">No AWS sandbox users yet</div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="min-w-full text-[13px]">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-200">
                                    {['Email', 'Username', 'Region', 'Status', 'Expires', ''].map(h => (
                                ...
                                {users.map(u => {
                                    ...
                                    return (
                                        <tr key={u._id} className={`hover:bg-gray-50/50 ${isDeleting ? 'opacity-50' : ''}`}>
                                            ...
                                            <td className="px-4 py-2.5 text-gray-500">
                                                {(u.expiresAt || u.endDate) ? new Date(u.expiresAt || u.endDate).toLocaleString('en-IN') : '-'}
                                            </td>
